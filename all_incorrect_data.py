...
class AllIncorrectData(BaseMethod):
    ...
    def find_similar_paths(self, clf, synth_paths, target_incorrect_path, importance):
    ...
    def find_synth_data_to_remove(self, synth_paths, target_paths, all_importances):
        ...
        for path_num in range(len(target_paths)):
            first_paths_similar_paths = self.find_similar_paths(first_paths_clf, first_paths, target_paths[path_num][0][0], all_importances[path_num][0])

            second_paths_similar_paths = self.find_similar_paths(second_paths_clf, second_paths, target_paths[path_num][0][1], all_importances[path_num][1])

            third_paths_similar_paths = self.find_similar_paths(third_paths_clf, third_paths, target_paths[path_num][0][2], all_importances[path_num][2])
    ...
    def experiment(self):
        ...
        items_to_remove = self.find_synth_data_to_remove(synth_paths, target_paths, all_importances)
